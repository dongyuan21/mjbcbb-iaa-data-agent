...
> 边界：本文是解释文档，不新增业务口径，不替代 `AGENT_RETRIEVAL_MAP.yaml`、表卡、语义层或 SQL 晋升门禁。
...
| 表卡层 | `ai_hive/agent_knowledge/tables/`、`ai_ck/agent_knowledge/tables/` | 表字段、分区、PII、freshness、join key、known pitfalls | 证明字段存在、表能不能用 |
...
| 待办队列 | `TODO/SQL写作链候选表准入积压清单.md` | 还没表卡化的候选表 | 告诉 Agent 哪些表还不能当稳定事实 |
| 建设计划 | `data_agent_plan/Text2SQL保守建设计划.md` | 保守建设路线 | 说明当前不做大而全 runtime |
| 回归门禁 | `eval/`、`tools/scripts/check_*` | consistency、表卡、路由、SQL 晋升、回归 | 防止把候选当事实、把草稿当 verified |
...
已 probe 表 -> ai_hive/agent_knowledge/tables/
字段证据模板 -> da_assets/
...
flowchart TD
  ...
  D --> E["读取通用 SQL 写作规则"]
  E --> F["按场景读取专项 policy"]
  F --> G["读取 semantic_contract / 表卡 / verified SQL"]
  G --> H["形成字段证据"]
  H --> I{"证据是否足够"}
...
data_agent_plan/Text2SQL保守建设计划.md
...
```text
帮我看下 BB GP 145 期商业化实验的插屏 ready 和收入。
```
...
- 表是否已经在 `ai_hive/agent_knowledge/catalog.yaml`。
- 是否有表卡。
- 分区字段是什么。
- 是否需要 `hour`。
- 是否必须过滤 `app_name`。
...
field_evidence:
  - requirement_item: 插屏收入
    ...
    evidence_source:
      type: table_card
      path: ai_hive/agent_knowledge/tables/xxx.yaml
...
| 结果 | 回写位置 |
|---|---|
| 表字段有新证据 | `ai_hive/agent_knowledge/tables/` 或 `ai_ck/agent_knowledge/tables/` |
...
ai_hive/agent_knowledge/tables/<相关表>.yaml
ai_ck/agent_knowledge/catalog.yaml
ai_ck/agent_knowledge/tables/<相关表>.yaml
```
...
| 用户问题 | 追加读取 |
|---|---|
| 商业化 / 广告链路 | `商业化SQL协议.md`、`白名单事件表查询协议.md`、`BB商业化埋点查询规则.md` |
...
当前 Text2SQL 是“受控写 SQL”。它必须先走规则、表卡、语义层和字段证据。

### 误区 2：DA 认证资料里的 SQL 可以直接 verified

不能。

DA 认证资料可以作为高质量业务线索，但 SQL 仍要经过表卡、schema、dry-run、小窗口和晋升门禁。
...
Text2SQL 是跨层工作流，不是独立资产目录。它消费 `knowledge`、`semantic_contract`、`ai_hive`、`ai_ck`、`da_assets`、`TODO`、`eval`。
...
| 想知道 | 看哪里 |
|---|---|
| Text2SQL 保守路线 | `data_agent_plan/Text2SQL保守建设计划.md` |
| Text2SQL 当前能力解释 | `data_agent_plan/Text2SQL当前能力说明.md` |
...
| 表字段和分区在哪 | `ai_hive/agent_knowledge/tables/`、`ai_ck/agent_knowledge/tables/` |
