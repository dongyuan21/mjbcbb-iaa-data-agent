...
| `skill_meta.json` | metadata_excluded | 无 | 仅为 skill 包元数据 / system prompt 包装，内容与已蒸馏规则重复；不作为项目知识来源 |
...
| `表信息.xlsx` | empty_excluded | 无 | 0 字节空文件；同名 md 已蒸馏 |
| `数据表索引.xlsx` | empty_excluded | 无 | 0 字节空文件；同名 md 已蒸馏 |
| `游戏产品名称.xlsx` | empty_excluded | 无 | 0 字节空文件；同名 md 已蒸馏 |
| `国家等级映射表.xlsx` | empty_excluded | 无 | 0 字节空文件；同名 md 已蒸馏 |
| `BB-GP商业化埋点查询文档.xlsx` | empty_excluded | 无 | 0 字节空文件；同名 md 已蒸馏 |
| `BB-IOS商业化埋点查询文档.xlsx` | empty_excluded | 无 | 0 字节空文件；同名 md 已蒸馏 |

## 源包收口结论

- `.md` 正文已逐篇蒸馏到本项目 policy、backlog、映射或专项协议；大 SQL 和字段清单只作为候选线索，不直接晋升 verified。
...
| 优先级 | 源文档 | 目标落点 | 处理要点 |
|---|---|---|---|
| P1 | `商业化实验主题表用法.md`、`白名单主题表使用方法.md` 表卡补充 | `ai_hive/agent_knowledge/tables/` | 已完成协议蒸馏；后续只补 live schema / 表卡可证实的字段、known_pitfalls 和 example_queries |
| P1 | `数据表索引.md`、`表信息.md` 候选表卡化 | `ai_hive/agent_knowledge/tables/`、`ai_hive/agent_knowledge/catalog.yaml` | 已完成路由蒸馏；只对通过 schema probe 的候选表补表卡 |
| P1 | `核心指标定义及SQL.md` SQL 样例后续拆分 | `knowledge/agent_knowledge/semantic_contract/metrics.yaml`、`da_assets/candidate_sql/` | 已完成口径蒸馏；只有通过字段证据和小窗口验证的 SQL 才能晋升 |
