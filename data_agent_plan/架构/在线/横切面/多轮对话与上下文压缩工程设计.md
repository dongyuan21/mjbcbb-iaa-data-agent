...
> 状态：active_architecture / partial_implementation（底层组件已实现，结构化状态真实回答链尚未闭合）
> 更新日期：2026-08-02
> 适用范围：Data Agent Runtime 的同步问答、SSE、durable job、Runtime Worker 和后续长会话
...
截至 2026-08-02，当前 Runtime 已具备：

1. session、原始消息、conversation turn、state checkpoint、compaction、job、trace 和 query record 的数据模型与 migration；
2. `ConversationState` 的 patch / clear、append-only correction、序列化、CAS checkpoint 和 session busy 底层实现；
3. rule TurnResolver、model resolver、conversation orchestrator、Context Compiler 和 L0–L4 reducer 的组件实现与单元测试；
4. Python `MinimalToolLoop` 的 route、knowledge、DataContract 和工具执行主链；
5. count-only 的 per-stage / context token 计量；
6. 原始消息历史兼容链：同步、SSE 和 Worker 可以读取认证 session 的已完成消息，明确追问和纠正表达可借历史文本续接；
7. rule TurnResolver 已在真实回答前执行并影响本轮 history injection / route，安全 metadata 会记录 turn_kind、reason 和 patch key。

### 2.2 仍未完成的能力

当前不能宣称“结构化多轮已工作”：

- `ConversationState` 和 checkpoint 类已经存在，但真实回答只写 `TurnResolution.to_safe_metadata()`；sync/SSE/Worker 的提交入口却读取从未生产的完整 `_turn_resolution_obj`，因此正常回答不会推进结构化 checkpoint。
- turn 开始时没有把 committed `base_state_version` 和 `turn_id` 作为同一内部合同贯穿到成功终态；orchestrator 会在缺参时读取回答结束时的最新版本，不能完整证明 stale-write fencing。
- `create_turn`、turn committed/failed ledger、session busy/enqueue、Context Compiler 和 reducer 目前没有生产主链调用者；组件存在和单测通过不等于主链已经接入。
- rule resolver 主要能生成短追问/替换 patch；普通首轮完整问题尚没有稳定的 product、platform、country、time、metric 全量结构化生产者。
- prior-slot 投影当前只回送 `entities`、`metrics`、`topic`，尚未统一投影 `time`、`constraints`、corrections、pending 和 evidence refs。
- `clarification_answer` 与 `cancel_or_restart` 虽在枚举中，尚未与 durable pending、语义重置和执行取消形成统一状态迁移合同。
- 原始历史注入仍是当前可用兼容链，因此简单追问可能成功；但确定性继承、纠正、切题、并发恢复和长会话压缩尚未获得真实 API/SSE/Worker 闭环证据。

对外统一口径应为：**原始历史多轮兼容链可用；结构化状态底层组件已实现，但真实回答提交链和 Context Compiler/压缩主链尚未闭合。**

### 2.3 成熟度必须分五个平面

本文后续出现“已实现”时必须明确属于哪一个平面，不再用一个“完成”覆盖全部状态：

| 能力 | 代码存在 | 组件单测 | 真实主链接入 | 已部署 | 线上/真实验收 |
|---|---|---|---|---|---|
| ConversationTurn / checkpoint 数据模型 | 是 | 是 | 否：生产目录无 create/commit turn 调用者 | 未核对 | 否 |
| ConversationState patch / CAS | 是 | 是 | 否：完整 TurnResolution 未送达提交端 | 未核对 | 否 |
| rule TurnResolver | 是 | 是 | 是：用于本轮分类和 history injection | 未核对 | 未形成结构化状态验收 |
| model resolver | 是 | 是 | shadow / enforce 受配置控制，默认不 enforce | 未核对 | 否 |
| conversation orchestrator | 是 | 是 | 调用点存在，但因完整对象缺失而直接返回 | 未核对 | 否 |
| Context Compiler / L0–L4 reducer | 是 | 是 | 否：生产目录没有调用者 | 未核对 | 否 |
| 原始消息历史兼容链 | 是 | 是 | 是 | 未核对 | 仅能证明有限追问，不等于结构化多轮 |

`未核对` 不是失败，也不能自动当成已部署；需要当前环境和 immutable provenance 证据后单独更新。

### 2.4 “结构化多轮状态”的三个不同状态空间
...
当前 `DataAgentRuntime` 已能计算 route、knowledge context 和 context metrics，但 `compile_context_plan()` 和 reducer 尚无生产调用者。接线时应通过 Context Compiler 集中管理 state、预算、压缩和安全投影，避免继续在 `agent.py` 中堆叠分支。
...
- sync /api/chat 端到端；
- SSE 端到端；
- durable job + MySQL + Runtime Worker 端到端；
...
| 验收面 | 必须能直接观察到 |
|---|---|
| sync API | 同一 session 的 `v0 → v1 → v2`，checkpoint 包含首轮完整 slot 与第二轮 patch value |
| SSE | 客户端断开/取消时 turn 终态正确，未成功终态不推进 version |
