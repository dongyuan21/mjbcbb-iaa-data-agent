...
>
> 关联事故：`sessions/12d2fe1c-3f5e-413f-ab1d-0638ee829a4f`
>
...
最大的改造点在 L4 规划主控：`MinimalToolLoop` 已成为唯一物理实现；逻辑上仍是**控制权归属**问题，不是重写表卡、SQL Guard、任务系统或数据底座。
...
flowchart TB
    subgraph MAIN["纵向主链：一次用户问题怎样变成可审计答案"]
        ...
        L6["L6 数据与知识底座<br/>MC / CK / 表卡 / 语义合同 / Verified SQL / Freshness"]
...
| 意图与路由 | route、fallback、clarification | route 只做软先验 |
...
| 受控发现 | catalog/search |
| 受控读取 | table card、policy、semantic contract、verified SQL |
| 上下文交付 | slim/fat context、budget、compaction、model-visible 标记 |

冻结边界：

- route 只参与排序，SafetyPolicy 才能拒绝资产；
- “retrieved”不等于“模型实际看见”，必须记录最终交付状态；
- 表卡继续作为当前权威数据合同入口，不引入第二套真相源；
...
| 开放规划 | 进程内 `MinimalToolLoop`（模型 ↔ 固定 9 工具） | 保持极简 loop，不引入 Coding Agent |
...
| 小层 | 当前/目标能力 |
|---|---|
| 工具边界 | Runtime Tool Registry 固定 9 工具 + 进程内 Python handler |
...
| 数据生产 | ETL、预聚合、预测与落表 |
| 数据合同 | 表卡、schema baseline、join keys、query rules |
...
- 本轮评审接受“数据底座和表卡能力是强项”这一前提；
- 不借 Runtime 改造重写数据生产、表卡体系或 semantic contract；
- 只有发现具体合同缺失时，按既有表卡/知识治理流程补齐，不新建平行体系。
...
| SQL 是否允许执行 | Execution & Safety / L5 | MinimalToolLoop、表卡、策略 | 模型自报 evidence ID 即获授权 |
...
- L6 的 MC/CK、表卡、语义合同、freshness 和 verified SQL 资产。
...
表卡、MC/CK、语义合同、verified SQL、freshness 体系继续作为既有基础，不因控制面收敛建立平行真相源。
...
2. 当前表卡/Registry 已具备强数据合同入口，缺的是在线 binding，不是必须换 OpenMetadata/DataHub；
