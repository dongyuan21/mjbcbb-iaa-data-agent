...
> 历史方案（已退役）：本文的轻量 LLM fallback 不在当前 Runtime 执行链路中；当前路由以确定性 `KnowledgeRouter` 与受控 `route_by_task_id` 重选为准。

> 状态：active_plan
> 创建：2026-07-24
> 更新：2026-07-26
> 范围：路由低置信度时的 fallback 通道、`search_catalog` 工具规格、route 命中率监控与长尾治理流程。
> 边界：本文件只定义检索/路由 fallback 策略和监控治理流程，不定义业务口径、不改数据资产目录内容、不涉及投放动作。
...
- ~150 张数据资产目录（数据资产目录 ~119 + 数据资产目录 ~30）
- ~60 个策略文件（policies / SOP / report_knowledge）
- ~50 个语义契约指标（semantic_contract/metrics.yaml）
- 18 条 task route 已覆盖生产问题模式的 90%+

对检索系统的核心要求是 **precision（不能召错表、不能用错口径）**，而非 recall（尽可能多召回）。召回错误的数据资产目录会让 LLM 用错 `must_filter` 和 `known_pitfalls`，后果比"没召回"更严重。
...
| chunk 破坏 | 把数据资产目录切成 512 token chunk 后，`query_rules` / `known_pitfalls` / `join_keys` 可能被切到不同 chunk，LLM 看不到完整约束 |
| 维护成本 | 更新数据资产目录后需要重新 embedding 全部 chunk；YAML 修改即生效，git diff 可审计 |
| 门禁 | chunk 内容不可验证；`check_da_metadata_quality.py` 可以校验每张卡的字段完整性 |
...
| B. 跨 route 组合 | 问题需要同时命中多个 route 的知识 | "示例产品 美国 DNU 下滑，是素材还是预算改了？" | 需要同时加载 `示例产品_us_ua` + `material` + `google_ads_config` |
...
search_catalog(query: string): array<{
  type: "da_metadata" | "policy" | "metric" | "entity" | "join",
  source: "数据资产目录" | "数据资产目录" | "knowledge" | "semantic_contract",
...
| 可搜索 | 不可搜索 |
|---|---|
| `数据资产目录/agent_knowledge/catalog.yaml` | `数据资产目录/audit_archive/` |
| `数据资产目录/agent_knowledge/catalog.yaml` | `数据资产目录/engineering_artifacts/` |
...
**判断标准**：如果监控数据显示 >10% 的问题命中了 primary route 但 LLM 在回答中需要搜索其他 route 的数据资产目录，则实现 composition_rules。否则不做。
...
| search_catalog 有效性 | 返回的 top-5 候选中，人工判断相关数据资产目录出现在 top-3 的比例 > 70% |
