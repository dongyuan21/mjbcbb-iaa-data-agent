...
- Runtime 本体：零代码，POC 触发条件已满足但未启动
- E2E 评测：现有回归只验证资产齐备性
- 北极星场景：46%（16/35）
...
| 框架 | 适配度 | 理由 |
|---|---|---|
| **Pydantic AI** | 高 | type-safe 结构化输出 + model-agnostic + 可测试；匹配 SQL 匹配→执行→结构化结果的形状 |
| LangGraph | 中偏高 | 最成熟的状态机编排；当前 POC 阶段过重，未来可叠加 |
| OpenCode | 不适合 | 编码 Agent（172K stars），用于写代码/调试，不是构建业务 Agent 产品的框架 |
| 完全自研 | 中 | 贴合但造轮子成本高 |

### 已确认：Pydantic AI + 自定义路由层

```
用户问题
 → 自定义路由层（AGENT_RETRIEVAL_MAP + task_routes）
 → Pydantic AI Agent（tool calling + 结构化输出）
   ├── MC 执行器（PyODPS）
   ├── CK 执行器（clickhouse-driver）
   └── 知识检索器（表卡 + verified SQL + 语义契约）
 → 门禁层（freshness / PII / read-only）
 → 结构化输出（output_contract）
```

路由层自己写：已有完整的 AGENT_RETRIEVAL_MAP.yaml 和 9 类 task_routes/，比任何框架的 Router 都贴合业务。

Agent 核心用 Pydantic AI：tool calling + 结构化输出 + model-agnostic + 可测试。
...
Pydantic AI 原生支持 OpenAI 兼容接口，零适配成本。

### 未来迁移路径

```
现在：自定义路由 + Pydantic AI（单 Agent）
  ↓ 需要多轮审批 / 多 Agent 协作时
未来：LangGraph 编排 + Pydantic AI Agent 节点
```
...
**SSE 断线重连**
- SSE 原生支持 `Last-Event-Id` 自动重连
- 前端加 heartbeat 检测（30s 间隔）
- 服务端从断点续传（配合 session store）
...
- Pydantic AI 最小 Agent：1 个 CK 查询 tool + verified SQL 匹配
- FastAPI + SSE 最小服务
- LiteLLM 网关接通验证
- structlog 日志
