...
| 适用范围 | ClickHouse(ROI360 看板) 与 MaxCompute(cohort 归因) 两类底座均验证可用 |
| 关联案例 | `../../ai_ck/audit_archive/mi_roi360_board_recon/`、`../../raw_exports/bb_arpu验证/recon_report.md` |
...
2. **反推口径(关键)**：口径不靠猜，按可信度优先级找真理源 —— 生产代码(如 nexus handler) > `knowledge/agent_knowledge/semantic_contract/model.json` > `ai_ck`/`ai_hive` 表卡 > verified_sql。拿到「报告字段 → 底层字段 → 计算口径」的精确映射。
...
6. **沉淀**：稳定 SQL → `verified_sql/`；完整现场证据 → 对应知识库 `audit_archive/evidence/`(MC 侧)或 `ai_ck/audit_archive/`(CK 侧)；订正/补充 → 表卡 `known_pitfalls` + 维度字典；并双向回链。
...
- 口径真理源：nexus 后端代码、`../../knowledge/agent_knowledge/semantic_contract/model.json`、`../../ai_ck/agent_knowledge/tables/*.yaml`、`../../ai_ck/agent_knowledge/metrics/`。
...
|---|---|---|---|---|
| `mi_roi360_board_recon` | ROI360 看板逐行(14781 行) | ClickHouse | 求和/留存 100%、ROI 实际段 97~98% | 历史快照 vs 当前回填 |
| BB_ARPU 对账 | 首日 ARPU 归因 | MaxCompute cohort | 激励 ARPU 逐日一致、趋势 100% | pv/非激励收入数据源口径 |
