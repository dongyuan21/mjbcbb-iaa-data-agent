...
| 来源 | `data_agent_plan/POC01_BB美国UA_DNU拆解路线.md`、`../verified_sql/vsql_20260618_google_campaign_dnu_changelog_triage.md` |
| 适用问题 | Google DNU 下滑、Google campaign 掉量、是否存在预算/状态/定向/ad_group/ad 操作记录 |
| 适用范围 | MaxCompute `dwd_market_appsflyer_activation_push_data_di` + `ods_market_google_ads_config_wide_hi` |
| 最后验证 | 2026-06-18 |
...
| 输入 | 说明 |
|---|---|
| `bundle_id` | 例如 `com.kcolb.juggle`。 |
| `country` | 例如 `US`；无国家限制时需明确为全量。 |
| `baseline_start` / `baseline_end` | 对照窗口，建议同 weekday 或前 7 天。 |
| `baseline_days` | baseline 日期数；前 7 天填 `7`。 |
| `anomaly_day` | 异常业务日期。 |
| `dt_scan_start` / `dt_scan_end` | activation 分区扫描窗口，必须按 `sop_20260618_dnu_install_time_dt_lag_guardrail` 扫到 latest 或至少 T+1。 |
| `change_dt_start` / `change_dt_end` | change log 分区日期，通常覆盖异常日前后。 |
...
2. 按 `sop_20260618_dnu_install_time_dt_lag_guardrail` 检查 activation 表 latest partition，设定 `dt_scan_end`。
...
| Google Ads change_event 宽表卡 | `../../ai_hive/agent_knowledge/tables/ods_market_google_ads_config_wide_hi.yaml` |
...
2026-06-18 验证窗口中，BB GP / US / Google 的 `2026-06-17` DNU 异动联查显示：

- `US-LH015-3.0--ARO-XH-260319-通用词`：DNU delta `-612.57`，同日命中 1 条 `CAMPAIGN_BUDGET` 事件，预算 `21000000000 -> 25500000000`；这不是预算下调证据。
