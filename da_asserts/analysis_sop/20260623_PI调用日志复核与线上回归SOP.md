...
| 项 | 内容 |
|---|---|
| ID | `sop_20260623_pi_call_log_review_and_online_rerun` |
| 状态 | verified |
| 来源 | `eval/PI调用日志复核_20260623/20260623_复核总报告.md`、`eval/PI调用日志复核_20260623/20260623_MC_CK补充复核.md`、`bugfix_doc/20260623_PI路由误判与空结果掩盖重大问题.md` |
| 适用问题 | PI / Data Agent 调用日志复核、失败样本诊断、线上回归验证 |
...
- 用户要求“复核 PI / Data Agent 日志”“看今天的 trace / 调用日志”“复跑线上问题”。
- 问题涉及 PI 回答与工具执行不一致、答非所问、空结果、错表、错 join key。
...
| MC 必跑 | 必须跑 `SELECT 1`，再跑至少一张目标表的 `max(dt)` / `max(active_date)` probe。 |
...
| 推荐入口 | 优先使用 `python3 tools/scripts/pi_review_tool.py review-day` 和 `python3 tools/scripts/pi_review_tool.py replay`，避免手工拼 SSO / MySQL / HTTP。 |
...
1. 读取 MySQL 日志：
   ...
   - 可直接运行：`python3 tools/scripts/pi_review_tool.py review-day --date 2026-06-24`
2. 建立 live 读取能力：
   - MC：`SELECT 1` + 目标表 `max(dt)`。
...
5. 需要改 runtime 时：
   - 先本地 E2E 跑通同题。
   - 再发布到 test 环境线上复跑。
   - 推荐使用：`python3 tools/scripts/pi_review_tool.py replay --question "<原问题>" --expect-route text2sql_or_sql_planning`
6. 线上复跑后必须检查：
   - 路由是否正确
   - PI 是否真实调用 MC / CK
...
```text
复核指定日期的 PI / Data Agent 调用日志，完成 MC/CK live 验证、失败样本诊断、必要的 runtime 修复、test 环境线上复跑与报告沉淀
```
