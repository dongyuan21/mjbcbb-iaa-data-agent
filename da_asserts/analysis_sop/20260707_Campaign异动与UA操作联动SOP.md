...
| 来源 | 钉钉文档《Campaign 全览计划详情页宣讲》、PGP `pgp/campaign-overview` 代码、`Campaign全览分析协议.md`、`20260705_投放看板模块知识蒸馏SOP.md` |
...
- 单个 Campaign 的 ROI360、消耗、CPI、DNU、注册、预估利润、ROI 倍率、素材、国家或 cohort 异动。
- 单个 Campaign 某段时间的数据、表现、趋势、日报或阶段复盘；若窗口内有 UA 操作，必须联动展示。
- 用户明确问 UA 历史、操作历史、调价、控量、放量、预算、状态、出价、素材调整后的效果。
- PGP Campaign 全览页面里的 `UA 历史`、`关键操作影响`、`ROI360 与消耗` 时间轴解释。
...
| 操作复盘入口：UA 调价 / 控量 / 放量 / 改预算 / 改状态 / 备注后怎么样 | 操作日前后 ROI360、消耗、CPI、利润、DNU / 注册、国家 / 素材结构 | “操作后数据是否朝预期方向变化，但不直接声称因果” |
...
| `bundle_id` | PGP / ROI360 scope 必填；不能猜包体 |
...
| `metric_set` | 默认 `ROI360 / cost / CPI / estimated_profit / ROI360_ROI1_multiplier`；DNU 问题追加 installs / registers |
| `revenue_source` | 默认 `sdk`，不确定时 SDK / AF 并列或标 `revenue_source_uncertain` |
| `forecast_version` | BB 默认 `v6_sdk`，非 BB 使用页面 / MI 当前可用版本；必须输出来源 |
...
默认使用 `pre_3d` / `post_3d` 是为了和 PGP `CampaignDataOverview.vue` 的 `buildOperationImpact()` 保持一致。若数据太稀疏、日耗低、cohort 未成熟，应升级为 `pre_7d` / `post_7d` 或只标观察。
...
|---|---|
| `cost` | 窗口内折后消耗求和 |
| `registers` / `DNU` | 窗口内求和；DNU 必须排除 `dt` 迟到分区风险 |
| `CPI` | `sum(cost) / sum(registers)` |
| `ROI360` | 优先 `sum(revenue_359) / sum(cost) * 100`；无 revenue 时按 cost 加权 ROI |
| `estimated_profit` | `sum(cost * (ROI360 / 100 - recycle_target_line))` |
| `ROI360 / ROI1` | 窗口内日级倍率均值，要求 ROI1 分母非 0 |
...
| 需求 | 默认来源 | 说明 |
|---|---|---|
| PGP 页面级 Campaign 指标 | PGP Campaign Overview / CK ROI cohort / MI ROI360 | 优先复用已验证页面链路；当前代码存在 CK-first ROI cohort 路径 |
| UA 历史 / 备注 / 操作 | PGP `POST /v1/campaign-overview/history` / MI campaign history | 适合页面级复盘 |
| Google 平台配置变更 | `h-s.ods_market_google_ads_config_wide_hi` | 需要 `customer_id + campaign_id`，只输出聚合变更 |
...
3. 拆主指标：
   - ROI360 下滑：看 cost scale、CPI、ROI 倍率、留存、国家、素材。
   - DNU / registers 下滑：看 media / country / campaign / DNU 迟到分区。
   - 消耗突变：看 ROI / CPI / 利润是否同向改善或恶化。
4. 查 UA 历史：
   - PGP 页面级先查 `/v1/campaign-overview/history`。
...
4. 补指标变化：
   ...
   - 出价 / tROAS / tCPA：看 CPI、ROI360、registers 是否朝预期变化。
...
1. 先输出用户所问时间段的 Campaign 主数据：ROI360、消耗、CPI、预估利润、DNU / 注册、ROI 倍率，必要时补素材和国家结构。
...
口径与参数：
campaign=<...>，bundle_id=<...>，media_source=<...>，date=<...>，revenue_source=<...>，forecast_version=<...>。

数据新鲜度与成熟度：
freshness_status=<...>，maturity_status=<...>，sample_status=<...>。

主数据事实：
1. 异动先发生在 <ROI360 / cost / CPI / DNU / 素材 / 国家 / cohort>。
2. 异动幅度：<baseline> -> <anomaly>。

UA 操作联动：
| 操作时间 | 操作类型 | 调前窗口 | 调后窗口 | cost | ROI360 | CPI | 预估利润 | 候选标签 |
|---|---|---|---|---:|---:|---:|---:|---|
