...
field_evidence:
  - requirement_item: ""
    ...
    expression: ""
    evidence_source:
      type: verified_sql | semantic_contract | da_metadata | live_schema | dry_run | small_window | knowledge_policy | candidate_backlog | owner_confirmed
      path: ""
      detail: ""
...
- 字段只来自源材料示例 SQL，未被数据资产目录、schema 或小窗口验证。
- 表未进入 `数据资产目录/agent_knowledge/catalog.yaml` / `数据资产目录/agent_knowledge/catalog.yaml`，且没有 live schema 证据。
- join key 没有数据资产目录、语义层或 owner 证据。
