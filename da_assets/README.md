...
>
> **与 `knowledge/` 的边界**：`da_assets/` 是"沉淀管线+过程产物"，`knowledge/` 是"已确认事实基座"。
...
da_assets/
  ...
  SQL写作链沉淀台账.md # sql-writing-chain 沉淀归属台账（自 dap 迁入）
  SQL写作链旁路验证审计.md # 源包审计与 probe 优先级（自 dap 迁入）
...
- 文档中的图片通常会以 Markdown 图片链接或 `unknown` block 出现；这只能证明“这里有图”，不能证明图中数值已经被读取。
