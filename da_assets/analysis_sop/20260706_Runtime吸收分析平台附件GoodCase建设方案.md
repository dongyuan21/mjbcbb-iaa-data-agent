# Runtime 吸收 分析平台 附件 Good Case 建设方案

## 元信息

| 项 | 内容 |
|---|---|
| ID | `sop_20260706_runtime_absorb_分析平台_attachment_good_case` |
| 状态 | `draft_for_runtime_absorption` |
| 来源 case | `../decision_cases/20260704_示例产品_GP_分析平台周六消耗下降附件分析案例.md` |
| 适用对象 | Data Agent Runtime、Runtime reviewer、真实问题验收集维护 |
| 目标 | 让 Runtime 把 分析平台 附件型业务问题组织成可信分析链，而不是只复述附件或直接查数 |
...
3. 识别证据结构：两个 Excel 是同一批 分析平台 数据的不同粒度。
...
9. 输出下一跳验证：live 分析平台 / CK / 操作日志 / owner decision。
10. 交付并回填：把分析写入钉钉文档，并沉淀成 case。

Runtime 要吸收的是这条可信分析链，而不是背诵本次 7.4 的具体数值。

## Runtime 触发条件

满足以下任一条件时，Runtime 应启用本方案：

- 用户上传或引用 分析平台 导出 Excel，并要求解释消耗、ROI、注册、CPI、DNU 或 campaign 变化。
...
| 步骤 | Runtime 必须做什么 | 验收标准 |
|---|---|---|
| 1. 问题识别 | 判断是否为 分析平台 / 投放消耗下降归因问题 | 输出或 trace 中有 `question_type=分析平台_spend_drop_diagnosis` |
...
| 10. 下一跳验证 | 生成 live 数据和操作记录复核清单 | 必须包含 `live_分析平台_or_ck_validation`、`operation_log_check`、`needs_decision` |
| 11. 交付回填 | 若写入文档或 case，记录 URL、附件、证据表和状态 | index / case 中保留 source、output、status |

## Answer Contract

Runtime 回答这类问题时，至少包含以下模块：
...
Runtime 不需要暴露原始思维流，但应在 trace / metadata / 答案摘要中保留可审计路径：

| Trace 字段 | 示例值 | 用途 |
|---|---|---|
| `question_type` | `分析平台_spend_drop_diagnosis` | 判断问题类型 |
| `input_source_type` | `分析平台_excel_export` | 区分附件、live SQL、MI API |
...
| `next_hops` | `live_分析平台_or_ck_validation,operation_log_check,owner_decision` | 后续验证 |
...
```json
{
  ...
  "optional_columns": ["国家和地区", "累计变现收入$", "分析平台"]
}
```
...
该 case 不要求 Runtime 复现完全相同数值，但要求 Runtime 产出同样的分析结构和证据边界：
...
本轮先不改代码。若要把这个 good case 吸收到 Runtime，建议拆成以下工程任务：

| 优先级 | 任务 | 验收方式 |
|---|---|---|
| P0 | 在 Runtime prompt / route profile 中加入 分析平台 附件型消耗下降 answer contract | `RWC_20260706_001` 回归通过 |
...
| P1 | 增加下一跳推荐器：Google Ads 操作日志、live 分析平台/CK、owner decision | 答案包含 pending checks |
| P2 | 钉钉输出与 case 回填自动化 | 文档 URL、case path、index entry 可追溯 |

## 与现有 Runtime 复核 SOP 的关系

- `Runtime查询执行复核护栏.md` 解决“查数 / SQL / 0 行 / query_records”执行质量。
- `20260623_Runtime调用日志复核与线上回归SOP.md` 解决“复核线上 trace / replay / 修 Runtime”。
- 本 SOP 解决“Runtime 如何吸收一个高质量业务分析 good case，并把它变成 answer contract 和 runtime 能力要求”。

三者关系：

```text
good case -> 本 SOP 定义答案合同和推理路径
answer contract -> 真实问题验收集约束 Runtime 输出
Runtime 执行 / replay -> 用现有 Runtime 复核 SOP 检查 trace、工具和落库
```
