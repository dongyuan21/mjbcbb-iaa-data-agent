...
| 档位 | 场景 | Runtime max_turns | bash 上限 |
|---|---|---:|---:|
| `default` | 常规问答/SQL | 20 | 6 |
| `long_report` | 本 SOP（读报表或跑 1 次脚本） | 12 | 3 |
| `long_investigation` | Runtime 日志复核等 | 40 | 20 |

环境变量（`runtime/backend/.env`）：`DATA_AGENT_MAX_TURNS_LONG_REPORT`、`DATA_AGENT_LONG_SCRIPT_TIMEOUT_SEC=900`
