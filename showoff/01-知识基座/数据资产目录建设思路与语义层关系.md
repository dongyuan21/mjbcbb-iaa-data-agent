...
`数据资产目录/` 是 MaxCompute / Hive 表的“使用说明书集合”。它告诉 Agent：这张表解决什么问题、按什么粒度记录、必须带哪些分区、哪些字段不能输出、可以和哪些表关联、有哪些常见误用。
...
```mermaid
flowchart TB
  Card["数据资产目录 数据资产目录<br/>表名、字段、粒度、分区、关联键、风险提示"]
...
```mermaid
flowchart LR
  Hive["数据资产目录<br/>事实源、明细、训练、标签、复核"]
  Ck["数据资产目录<br/>MI 报表层、分析平台、OLAP 查询"]
