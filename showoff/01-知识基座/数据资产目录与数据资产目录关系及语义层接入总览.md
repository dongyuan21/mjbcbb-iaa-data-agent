# 数据资产目录 与 数据资产目录 关系及语义层接入总览

> 汇报定位：用一份总览材料讲清楚 MC 事实源、CK 报表层、语义层、分析资产层之间的关系。
> 适合场景：当听众问“为什么要同时建 数据资产目录 和 数据资产目录，它们和语义层到底是什么关系”时使用。

## 先给答案

`数据资产目录` 和 `数据资产目录` 都是数据资产目录层，但服务不同数据源：

- `数据资产目录` 管 MaxCompute / Hive，更接近底账、明细、训练、标签和事实复核。
- `数据资产目录` 管 ClickHouse / MI，更接近看板、分析平台、OLAP 聚合和页面口径复现。
...
flowchart TB
  ...
  CK["ClickHouse / MI<br/>报表、分析平台、OLAP"]
  Hive["数据资产目录<br/>MC 表使用说明"]
  Ck["数据资产目录<br/>CK 表和页面口径说明"]
...
| 适合问题 | 明细复核、标签、训练、S2S、AF ODS、预测偏差 | 分析平台、消耗、回收、留存、campaign 映射、页面复现 |
...
```mermaid
flowchart LR
  HiveCard["数据资产目录 数据资产目录<br/>表名、字段、分区、join"]
  CkCard["数据资产目录 数据资产目录<br/>表名、指标、日期、join"]
...
flowchart LR
  ...
  Card["2. 写清数据资产目录<br/>粒度、日期、PII、关联键、风险提示"]
