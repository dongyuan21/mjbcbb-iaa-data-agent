...
flowchart TB
  ...
  subgraph RuntimeView["运行层展示视图"]
    Runtime["runtime/backend FastAPI<br/>+ MinimalToolLoop<br/>+ runtime/deploy 山海 K8s"]
  end
  ...
  subgraph KnowledgeView["知识基座展示视图"]
    Hive["ai_hive<br/>MaxCompute 表卡"]
    Ck["ai_ck<br/>ClickHouse/MI 表卡"]
...
flowchart TB
  ...
  Candidate["候选资产<br/>候选 SQL / 草稿流程 / L0-L1 表卡"]
  Validate["验证<br/>小窗口跑数、schema probe、对账"]
  Verified["可信资产<br/>已验证 SQL / L2-L3 表卡 / SOP / case"]
...
flowchart LR
  ...
  Task --> First["L3 first_read<br/>协议 + 表卡 + SOP + 阈值"]
  First --> Card["L4 命中表卡<br/>不整包加载"]
...
flowchart TD
  ...
  Route --> Assemble["7. 初始上下文装配<br/>first_read、表卡摘要、verified SQL"]
  Assemble --> Loop["8. MinimalToolLoop<br/>模型 ↔ 固定 9 工具"]
