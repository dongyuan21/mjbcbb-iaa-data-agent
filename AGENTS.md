...
本目录是面向 Data Agent / 自研开源 DataAgent runtime 的投放、点位、MI ROI360、MaxCompute、ClickHouse 知识工作区。本目录为**独立工作区**，自带完整常驻规则；父级 `../AGENTS.md`（历史点位 Excel→MySQL 导入工作）已通过 frontmatter `globs` 限定到 `点位/` 本目录的导入文件，不再注入数仓知识库上下文。

## Always Applied Rules

1. 默认使用中文简体。
2. 后续由 AI 新建的 `.md` 文档默认使用中文文件名；若因外部约定、工具识别或既有固定入口必须使用英文文件名，需在回复中说明原因。
3. 不把 `raw_exports/`、`da_assets/raw/`、`knowledge/audit_archive/draft_knowledge/` 中的内容直接当 verified 事实。其中 `raw_exports/` 与 `da_assets/raw/` 仅本地归档、不纳入 git（见 `.gitignore`）；文档/表卡中指向它们的路径是本地来源标注，不是仓库内可达文件。
...
8. 评估 `ai_hive` / `ai_ck` / Data Agent 数据基座、数据新鲜度、表质量或问答回归前，必须先建设并验证 MC 与 CK 读取能力：
   ...
   - 能力验证成功后，必须重跑 `tools/scripts/probe_freshness.py` 刷新 `ai_ck/engineering_artifacts/freshness_snapshot.json` 和 `ai_hive/engineering_artifacts/freshness_snapshot.json`，再跑必要的 agent regression。
   - 回答中必须区分“实时探测结果”和“读取旧快照/旧报告得到的结果”。
9. 修改 `ai_hive/agent_knowledge/tables/*.yaml`、`ai_ck/agent_knowledge/tables/*.yaml` 或相关 catalog / PII policy 后，必须运行：
...
10. 新增 Hive / MaxCompute 表到 `ai_hive/` 时，必须按 `ai_hive/engineering_artifacts/Hive表准入标准.md` 执行。DDL、血缘、生产 SQL / run 日志只是证据来源；进入 Data Agent 默认召回前还必须补齐 `query_rules`、PII、freshness、`join_keys`、`known_pitfalls` 和安全 `example_queries`。
11. 修改 `knowledge/` 下任意文件后，必须同步 `knowledge/agent_knowledge/catalog.yaml` 并运行：

   ```bash
   python3 tools/scripts/check_knowledge_consistency.py
   ```

   硬错误和 warning 必须清零；不得把 `knowledge/audit_archive/draft_knowledge/`、`snapshot`、`raw` 或未覆盖当前周期的历史知识加入默认召回。
12. 目录分层、manifest 约定与跨库归属以 `data_agent_plan/知识库三层结构规范.md` 为唯一 canonical 定义；新增知识库 / 新层 / 新顶层目录先改规范并在 `AGENT_RETRIEVAL_MAP.yaml` 完成分类，再落地。修改 `ai_ck/agent_knowledge/metrics/` 或 `knowledge/agent_knowledge/semantic_contract/metrics.yaml` 后，必须运行：

   ```bash
   python3 tools/scripts/check_metric_layer_boundary.py
   ```

   该门禁已纳入 `eval/agent_regression/run_regression.py`；跨表机器指标只归 `semantic_contract/metrics.yaml`，CK / MI 页面指标解释只归 `ai_ck/agent_knowledge/metrics/`，二者不得复制同一公式或 join 定义。
...
- 涉及 BB GP / iOS 大埋点字典、非商业化事件、APM、push、支付、web / H5、玩法 UI 或端差异字段时，同时读取 `knowledge/agent_knowledge/policies/BB大埋点字典使用规则.md`。
- 涉及 AB3.0 `fs` / `rv` / `ba` 实验 ID 时，同时读取 `knowledge/agent_knowledge/policies/AB3实验ID提取规则.md`。
- 涉及小包兜底 / 中价 / 高价广告单元档位时，同时读取 `knowledge/agent_knowledge/policies/小包广告单元映射.md` 和 `knowledge/agent_knowledge/policies/小包广告单元映射.csv`。
- 涉及模型特征、模型输出标签、事件定义、参数定义、用户属性定义或 Hudi 字段映射时，同时读取 `knowledge/agent_knowledge/policies/特征工程与埋点元数据查询规则.md`。
- 涉及投放、AF 激活、SKAN、成本、SDK 收入、campaign / adset / ad、ROAS / ROI 或 ROI 预估时，同时读取 `knowledge/agent_knowledge/policies/投放与ROI预估SQL协议.md`。
